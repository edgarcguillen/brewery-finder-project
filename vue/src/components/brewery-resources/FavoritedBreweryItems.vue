    ...
    <div class="base-card">
      ...
      <div class="card-slot" id="info-section">
        <div class="add-favorite" v-if="isUser">
          <button
            type="favorite"
            v-on:click="removeFavorite(attrs.breweryID)"
            id="fav-button"
          >
            <!-- Bring in method to determine if brewery is in favorites -->

            <img src="../../assets/Heart-Liked.png" id="heart-icon" />
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    removeFavorite(id) {
      breweryService.removeBreweryFromFavorites(this.$store.state.user.id, id);
      location.reload();
    },
    ////////////////////////////////////////////////////////////////
    retrieveBreweries() {
